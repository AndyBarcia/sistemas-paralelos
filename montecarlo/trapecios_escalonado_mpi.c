...
int main(int argc, char **argv) {
	...
	// Calcular el punto inicial en el que evaluará este proceso,
	// y el step en base al número de iteraciones.
	double start_x = ((double) node*iterations) / iterations_total;
    double step_x = (1.0 / npes) / iterations;

	// Evaluar el primer punto
	double prev_f = 1.0 / sqrt(1.0 - start_x*start_x);
	for (long i = 1; i < iterations; ++i) {
		// Ignorar punto que evalua a infinito.
		if (node == npes-1 && i == iterations -1) continue;

		// Evaluar el segundo punto
		double x = start_x + step_x*i;
		double actual_f = 1.0 / sqrt(1.0 - x*x);

		// Sumar area del trapecio entre el primer y segundo punto.
        total_local += step_x * ((actual_f + prev_f) / 2.0);
        prev_f = actual_f;
	}
